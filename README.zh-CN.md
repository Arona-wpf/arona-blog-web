...
Aronaの小屋 网站的前端 Web 服务。功能还在逐步开发中……
...
- **表单命名**：`name` 属性使用 `lowercase_snake_case` 格式，如 `login_form`、`register_form`、`reset_step_1_form`
- **多表单拆分**：当一个页面包含多个独立表单（如多步骤流程），必须将每个表单拆分为独立的 `.vue` 文件，避免表单冲突
